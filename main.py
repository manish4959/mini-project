...
window.geometry('1280x720') window.configure(background=bgcolor) #window.attributes('-fullscreen', True)
window.grid_rowconfigure(0, weight=1) window.grid_columnconfigure(0, weight=1)
message1 = tk.Label(window, text="Traffic Prediction" ,bg=bgcolor ,fg=fgcolor ,width=50 ,height=3,font=('times', 30, 'italic bold underline'))
...
def browse1(): path=filedialog.askopenfilename() print(path)
txt1.delete(0, 'end') txt1.insert('end',path)
if path !="":
                  print(path)
            else:
tm.showinfo("Input error", "Select Image")
def browse2(): path=filedialog.askopenfilename() print(path)
txt2.delete(0, 'end') txt2.insert('end',path)
if path !="":
                  print(path)
            else:
tm.showinfo("Input error", "Select Video")
      def trainprocess():
            ...
            if sym != "" :
                  TR.process(sym,4)
tm.showinfo("Input", "Training Successfully Finished") else:
...
img = cv2.imread(sym)
font = cv2.FONT_HERSHEY_SIMPLEX cv2.putText(img, str(result) ,(19,51), font,
1,(0,0,255),5,cv2.LINE_AA)
                  cv2.imwrite("output.jpg", img)
plt.imshow(image.load_img("output.jpg")) plt.show()
#plt.imshow(image.load_img(np.random.choice(image_files))) #plt.show()
else:
